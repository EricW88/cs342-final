...
INFINITY = float('inf')
file = "/Users/richardliu/School/S2022/CS342/Homework/richard-final/tournament/images/"
...
def is_puck_behind_kart(kart_location, kart_front, puck):

    kart_x = kart_location[0]
    kart_z = kart_location[2]

    kart_front_x = kart_front[0]
    kart_front_z = kart_front[2]

    puck_x = puck[0]
    puck_z = puck[2]

    if kart_x == kart_front_x:
        # kart is straight up and down, perpendicular to goal
        return puck_z < kart_front_z and kart_z < kart_front_z or \
            puck_z > kart_front_z and kart_z > kart_front_z
    
    if kart_z == kart_front_z:
        # kart is sideways parallel to goal
        return puck_x < kart_front_x and kart_x < kart_front_x or \
            puck_x > kart_front_x and kart_z > kart_front_x


    #print(str((kart_x, kart_z)))
    # find the line between the kart and the front of the kart to determine its orientation
    linear_coefficients = slope_intercept(kart_x, kart_z, kart_front_x, kart_front_z)

    #print("points: " + str([kart_x, kart_z, kart_front_x, kart_front_z]) + " coeff: " + str(linear_coefficients))

    kart_slope = linear_coefficients[0]
    kart_y_intercept = linear_coefficients[1]

    if kart_slope == 0:
        perpendicular_slope = INFINITY
    else: 
        perpendicular_slope = -1 / kart_slope

    # if the front of the kart and puck are both in the same region, the puck is behind the kart
    if puck_z < perpendicular_slope * puck_x + kart_y_intercept and kart_front_z < perpendicular_slope * kart_front_x + kart_y_intercept or \
         puck_z > perpendicular_slope * puck_x + kart_y_intercept and kart_front_z > perpendicular_slope * kart_front_x + kart_y_intercept:
        return True
...
class Match:
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
        ...
        for it in range(max_frames):
            ...
            #print("goal lines: " + str(soccer_state["goal_line"]))
            ...
            ball_location = soccer_state["ball"]["location"]
            # (x,y,z) coordinate triple
            # second location is y, never changes

            team1_p1_image = team1_images[0]
            team1_p2_image = team1_images[1]
            team2_p1_image = team2_images[0]
            team2_p2_image = team2_images[1]

            images = [team1_p1_image, team1_p2_image, team2_p1_image, team2_p2_image]

            team1_p1_location = team1_state[0]["kart"]["location"]
            team1_p2_location = team1_state[1]["kart"]["location"]
            team2_p1_location = team2_state[0]["kart"]["location"]
            team2_p2_location = team2_state[1]["kart"]["location"]
            
            locations = [team1_p1_location, team1_p2_location, team2_p1_location, team2_p2_location]

            team1_p1_front = team1_state[0]["kart"]["front"]
            team1_p2_front = team1_state[1]["kart"]["front"]
            team2_p1_front = team2_state[0]["kart"]["front"]
            team2_p2_front = team2_state[1]["kart"]["front"] 

            kart_fronts = [team1_p1_front, team1_p2_front, team2_p1_front, team2_p2_front]

            #print("front: " + str(team1_p1_front))
            #print("fov: " + str(team1_state[0]["camera"]["fov"]))

            t1p1_image_coords = to_image(ball_location, (team1_state[0]["camera"]["projection"]).T, (team1_state[0]["camera"]["view"]).T)
            t1p2_image_coords = to_image(ball_location, (team1_state[1]["camera"]["projection"]).T, (team1_state[1]["camera"]["view"]).T)
            t2p1_image_coords = to_image(ball_location, (team2_state[0]["camera"]["projection"]).T, (team2_state[0]["camera"]["view"]).T)
            t2p2_image_coords = to_image(ball_location, (team2_state[1]["camera"]["projection"]).T, (team2_state[1]["camera"]["view"]).T)

            image_coords = [t1p1_image_coords, t1p2_image_coords, t2p1_image_coords, t2p2_image_coords]

            #print("puck location in frame " + str(it) + ": " + str(ball_location))
            plt.ylim([-20, 20])
            plt.xlim([-64.5, 64.5])
            print("Frame " + str(it))
            plt.scatter([x[0] for x in locations[:2]], [x[1] for x in locations[:2]], color='red', marker=".", s=50)
            plt.scatter([x[0] for x in locations[2:]], [x[1] for x in locations[2:]], color='blue', marker=".", s=50)

            plt.scatter([ball_location[0]], [ball_location[1]], color='black', marker=".", s=100)
            plt.savefig(file + "frame" + str(it) + 'locations.png')

            plt.clf()

            for i, image_coord in enumerate(image_coords):

                player_location = locations[i]
                kart_front = kart_fronts[i]

                puck_behind_kart = is_puck_behind_kart(player_location, kart_front, ball_location)

                # check if the image coordinate of the puck is in the screen
                #if puck_behind_kart:

                if image_coord[0] < 400 and image_coord[1] < 300 and not puck_behind_kart:
                    #print('player ' + str(i+1) +  " location: " + str(player_location))
                    #print('player ' + str(i+1) +  " kart front: " + str(kart_front))

                    file_name = file + 'player' + str(i) + "|frame" + str(it) + '.png'
                    Image.fromarray(images[i]).save(file_name)
                    img = Image.open(file_name)
                    d1 = ImageDraw.Draw(img)
                    d1.rectangle((image_coord[0] - 2, image_coord[1] - 2, image_coord[0] + 2, image_coord[1] + 2), fill=(255, 0, 0), outline=(0, 0, 0))
                    img.save(file_name)
